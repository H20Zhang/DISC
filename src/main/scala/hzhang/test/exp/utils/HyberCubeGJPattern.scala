...
//        new CompactEdgePatternLoader(rawEdge,Seq(hNumber._1,hNumber._2)) edgeLogoRDDReference
  ...
  def pattern(name:String)  ={
    name match {
      ...
      case "near5Clique" => near5CliqueNewOrder
  ...
  lazy val chordalSquare = {
    ...
    val edge3_1 = getEdge(5, 2)
    val edge3_3 = getEdge(5, 5)

    val triangle = edge3_3
//      .filter(p => p(0) < p(1))
      .build(edge3_3.to(1,2), edge3_3.to(0,2)).toConcrete()

    val chordalSquare = triangle.build(edge3_1.to(1,3), edge3_1.to(0,3))
//      .filter(p => p(2) < p(3))
...
//  optimal share
    lazy val house3 = {
      ...
      val edge6_6 = getEdge(6, 6)
      val edge6_3 = getEdge(6, 3)
      val edge3_2 = getEdge(3, 2)
      val edge6_2 = getEdge(6, 2)
      val edge6_1 = getEdge(6, 1)

      val leftEdge = edge6_6.filter(p => p(0) < p(1))
      val wedge = leftEdge.build(edge6_3.to(1,2))
      val squareTemp = wedge.build(edge3_2.to(2,3), edge6_2.to(0,3)).filter(p => p(0) != p(2) && p(1) != p(3))
      val house = squareTemp.build(edge6_1.to(1,4), edge6_1.to(0,4)).filter(p => p(3) != p(4) && p(2) != p(4))
...
//      .filter(p => p(3) != p(2) && p(4) != p(1) && p(3) != p(4))
  ...
  lazy val threeCenterTriangle = {
    ...
    val threeTriangle = chordalSquareTemp.build(edge4_1.to(0,4),edge4_1.to(1,4))
    //      .filter(p => p(3) != p(2) && p(4) != p(1) && p(3) != p(4))
  ...
  lazy val twoHeadTriangle = {
    ...
    val trianglePlusOneEdge= triangle.build(edge4_1.to(1,3))

    val twoHeadTriangle = trianglePlusOneEdge.build(edge4_1.to(1,4),edge4_1.to(3,4))

    twoHeadTriangle
  }

  lazy val near5Clique = {
    val filteredEdge = getEdge(4, 4)
  ...
  lazy val near5CliqueNewOrder = {
    ...
    val near5Clique = chordalSquare.build(edge3_3.to(0, 4), edge3_3.to(1, 4), edge3_3.to(2, 4)).filter(p => p(4) != p(2) && p(4) != p(3))
