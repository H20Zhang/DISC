...
class SymmetryBreakRule extends Rule {
  ...
  override def transform(elem: Element): Seq[Element] = {
    ...
    val transformedElement = Element(
      elem.V,
      elem.E,
      elem.C,
...
class InducedToNonInduceRule extends Rule {
  override def isMatch(elem: Element): Boolean =
  ...
  override def transform(elem: Element): Seq[Element] = {
    ...
    elements += Element(elem.V, elem.E, elem.C, elem.factor, State.Isomorphism)
...
class NonInduceToPartialRule extends Rule {
  override def isMatch(elem: Element): Boolean =
...
//    }
    ...
    //gen new elements
    val elements = ArrayBuffer[Element]()
    elements += Element(elem.V, elem.E, elem.C, elem.factor, State.Partial)
...
class CliqueOptimizeRule extends Rule {
  override def isMatch(elem: Element): Boolean = {
