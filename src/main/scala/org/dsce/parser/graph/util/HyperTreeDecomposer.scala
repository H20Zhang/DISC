package org.dsce.parser.graph.util

import org.dsce
import org.dsce.parser.graph.{Graph, HyperNode, HyperTree}
import org.dsce.parser.graph.{Graph, HyperNode, HyperTree}
import org.dsce.util.ExamplePattern.PatternName
import org.dsce.util.{ArrayUtil, ExamplePattern, GraphBuilder}
import org.dsce.{EdgeList, LogAble, TestAble}
import org.dsce.parser.graph
import org.dsce.parser.graph.{Graph, HyperNode, HyperTree}
import org.dsce.util.{ArrayUtil, ExamplePattern, GraphBuilder}
import org.dsce.util.ExamplePattern.PatternName
...
object HyperTreeDecomposer extends TestAble with LogAble {
  ...
  def allGHDs(g: Graph) = {
    ...
    extendableTree += ((dsce.graph.HyperTree(ArrayBuffer(), ArrayBuffer()), g.E()))
