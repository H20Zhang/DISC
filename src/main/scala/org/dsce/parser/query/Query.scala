...
import org.dsce.parser.remover._
import org.dsce.parser.visualization.VisualizeAble
import org.dsce.{org.dsce.parser.remover, _}
import org.apache.commons.lang.builder.{ToStringBuilder, ToStringStyle}
import org.dsce.parser.graph.Graph
import org.dsce.parser.graph.Graph
import org.dsce.parser.remover.{
  Element,
  Equation,
  NonInducedInstanceRemover,
  NonIsomorphismInstanceRemover,
  SymmetryBreaker
}
import org.dsce.parser.visualization.VisualizeAble
  ...
  def toEquation(stage: Stage): Equation = {
    stage match {
      case Stage.Unprocessed => {
        remover.Equation(toElement(), toElement(), Stage.Unprocessed)
      }
