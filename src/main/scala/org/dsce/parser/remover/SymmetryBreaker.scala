package org.dsce.parser.remover

import org.dsce.parser.query._
import org.dsce.util.{ArrayUtil, ExamplePattern, QueryBuilder}
import org.dsce._
import org.dsce.parser.graph.util.WidthCalculator
import org.dsce.util.ExamplePattern.PatternName
import org.dsce.util.ExamplePattern.PatternName
import org.dsce.util.{ArrayUtil, ExamplePattern}
  ...
  private def genCondition(matches: Seq[NodeID]): Seq[SymmetryBreakingRule] = {
    ...
    if (matches.size == 1) {
      ...
      matches.sorted.drop(1).map { f =>
        SymmetryBreakingRule(head, f, sortedIsoNodes)
      }
    }
  }
