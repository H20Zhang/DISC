package org.apache.spark.Logo.UnderLying.UniformSampler
...
class TriangleSampler(data:String) {
  ...
  lazy val edgeDataset = {
    val dataset = new EdgeLoader(data).EdgeDataset.cache()
    dataset.count()
    dataset
  }
...
//    Collect Total Degree
    val pointPartitionInfo = weightedPoint.mapPartitionsWithIndex{
      case(id, it) =>
...
//    print(s"total degree is :${totalDegree}")
    ...
    val SamplesPos = SparkSingle.getSparkContext().parallelize(Range(0, 200), 200).mapPartitions(f => Range(0, k/200).iterator)
...
//    There maybe some bugs when zipping the partitions
    ...
    val samples = rddSamples.map(f => (f,1)).aggregateByKey(0)(seqOp1, combineOp1).partitionBy(new HashPartitioner(200))
...
//    val samples = rddSamples.groupBy(f => f).map(f => (f._1, f._2.size)).partitionBy(new HashPartitioner(200))
  ...
  def wedgeSample(k:Int) = {
    ...
    val groupedEdges = edge.groupByKey(new HashPartitioner(200)).zipPartitions(pointSamples){
