...
//      .filter(f => f(0) != f(1)).map(f => (f(0), f(1))).flatMap(f => Iterator(f, f.swap)).toArray.distinct.map(f => Array(f._1, f._2))
  ...
  def genCatersianProductContent(domainSize:Int, arity:Int, SampleRate:Double = 0.5):Array[Array[Int]] = {
    var content = Range(0, domainSize).map(f => Array(f)).toArray


    for (i <- 0 until arity - 1){
      content = Range(0, domainSize).flatMap{
        value =>
          content.map{
            tuple =>
              tuple :+ value
...
//    println(domainSize)
//    println(arity)
//    println(s"domainSize:${domainSize}, arity:${arity}, contentSample:${content.head.toSeq}")
