...
object QueryGenerator {
  ...
  def genRandomQuery(numRelation:Int, arity:Int, cardinality:Int):(Seq[Array[Array[DataType]]], Seq[RelationSchema]) = {
    val domainSize = Math.pow(cardinality.toDouble, 1/arity.toDouble).ceil.toInt
    val attributes = Range(0, arity).map(_.toString)
    val subsets = Random.shuffle(SeqHelper.subset(attributes))

    val schemas = Range(0, numRelation).map{
      i =>
    ...
    val contents = schemas.map{
      schema =>
        ContentGenerator.genCatersianProductContent(domainSize, schema.attrIDs.size).distinct
    }
