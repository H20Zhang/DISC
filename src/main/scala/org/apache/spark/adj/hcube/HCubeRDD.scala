...
class SubTaskPartition(
  ...
  var partitionValues = dependencyBlockID.zipWithIndex.map(_.swap).map(f => (f._1, rdds(f._1).partitions(f._2)))
  val info = taskInfo
  ...
  def calculatePreferedLocation = {
    var prefs = dependencyBlockID.zipWithIndex.map(_.swap).map(f => rdds(f._1).preferredLocations(rdds(f._1).partitions(f._2)))
