...
class HCube(query: HCubePlan, info: TaskInfo) {
  ...
  def preprocessPartitionedRelation(
    partitionedRelations: Seq[PartitionedRelation]
  ) = {
  ...
  def genHCubeRDD() = {
    ...
    val preprocessedPartitionedRelation = preprocessPartitionedRelation(
      partitionedRelations
    )
...
case class HCubePlan(relations: Seq[Relation],
