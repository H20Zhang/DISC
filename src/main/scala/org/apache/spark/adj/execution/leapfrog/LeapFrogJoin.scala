...
import org.apache.spark.adj.execution.leapfrog.Intersection.seek
import org.apache.spark.adj.plan.LeapFrogJoinSubTask
...
//    val lastIterator = unaryIterators(lastIdx)
    binding(lastIdx) = unaryIterators(lastIdx).next()
    binding
  }
