...
import org.apache.spark.disc.execution.subtask.FactorizedLeapFrogJoinSubTask
...
class FactorizedLeapFrogJoin(task: FactorizedLeapFrogJoinSubTask)
  ...
  protected val contents =
    task.blocks.map(_.content).map(f => f.map(g => g.clone()))
  protected val schemas = task.blocks.map(_.schema)
  protected val numRelations = schemas.size
  protected val attrOrders = task.attrOrders
  ...
  var itCache: Array[Array[DataType]] = new Array[Array[DataType]](attrSize)
  protected val initV = {
//    println(s"factorize init")
    init()
  }
  ...
  protected def fixIterators(idx: Int): Unit = {
    ...
    while (hasEnd != true) {
      ...
      while (prevIterator.hasNext) {
        ...
        if (prevIdx > corePos && itCache(idx) != null) {
          it = itCache(idx).toIterator
...
//          itCache = new Array[Array[DataType]](attrSize)

          if (it.hasNext) {

            if (idx > corePos + 1) {
              itCache(idx) = it.toArray
              unaryIterators(idx) = itCache(idx).toIterator
            } else {
              unaryIterators(idx) = it
            }
