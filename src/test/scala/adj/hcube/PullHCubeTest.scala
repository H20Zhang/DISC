...
import org.apache.spark.adj.execution.hcube.{HCubeHelper, TupleHCubeBlock}
...
class PullHCubeTest extends FunSuite with BeforeAndAfterAll {
  ...
  test("HCubeHelper -- generate partitioner") {
    ...
    val isPartitionerValid = contentForR1.forall { tuple =>
      ...
      tupleHashValue == partitionerForR1.getPartition(
        partitionerForR1.getShare(tupleHashValue)
      )
    }
  ...
  test("RelationPartitioner") {
    val relationPartitioner = new RelationPartitioner(
      hcubePlan.idForRelation(testingRelationID),
      helper
    )
    val partitionedRelation = relationPartitioner.partitionRelation()
    val numOfBlocks = partitionedRelation
      .mapBlock { f =>
        Iterator(1)
      }
      .count()
    ...
    val partitionerForR1 = helper.partitionerForRelation(testingRelationID)
    val isPartitionCorrectly = partitionedRelation.partitionedRDD
  ...
  test("HCube -- generate hcubeRDD") {
    ...
    val hcube = new PullHCube(hcubePlan, taskInfo)
    ...
    hcube
      ...
      .flatMap { subtask =>
        ...
        println(s"subjoin share:${subjoin.shareVector.toSeq}, shares:${shareVectors
          .map(_.toSeq)}, attrsForeachRelation:${relationSchemas.map(_.attrIDs)}")

        blocks.combinations(2).map { blockPair =>
          ...
          if (!commonAttributes.isEmpty) {
            val isAccordingShareSame = commonAttributes.forall { attrID =>
      ...
      .collect()
      .foreach(
        p => assert(p, s"some thing wrong in distribute blocks in HCubeRDD")
      )

//    println(s"number of subtasks:${adj.hcube.genHCubeRDD().count()}")
