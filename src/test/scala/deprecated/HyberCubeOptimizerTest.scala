...
import org.apache.spark.adj.deprecated.plan.deprecated.LogicalPlan.Decrapted.HyberCubeOptimize.{HyberCubeOptimizer, PGenerator}
...
//    }
  ...
  test("HyberCubeOptimizer") {
    ...
    val hyberCubeOptimizer = new HyberCubeOptimizer(house, 1, 224, 5, Int.MaxValue)
    val possiblePlans = hyberCubeOptimizer.allPlans()

    if (!possiblePlans.isEmpty){
      val minCostPlan = possiblePlans.minBy(f => f.Load)
